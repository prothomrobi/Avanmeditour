...
// ********************************** add_hos_country ********************************
    ...
    public function add_doctor() {
        ...
        if($_POST){
            ...
            $this->back_temp->loadCont("back/home/doctor.php", array("lists" => $lists, "names" => $names));
        }
    }
    ...
    public function about_us() {
        ...
        if($_POST){
            ...
            if(empty($_FILES['img']['name'])){
                ...
                // config ------------------------------------------------------
                $config['upload_path']     = './uploads/front/about_us/';
                $config['file_ext_tolower'] = TRUE;
                $config['file_name']       = $avatar;
                $config['remove_spaces']   = TRUE;
                $config['allowed_types']   = 'jpg|png';
    ...
    public function update_about_us() {
        if ($_POST) {
            ...
            // renaming --------------------------------------------------------
            rename("./uploads/front/about_us/" . $old_img, "./uploads/front/about_us/" . $form['img']);
    ...
    public function replace_image_about_us() {
        if ($_POST) {
            // recv post data --------------------------------------------------
            $id = $this->common->nohtml($this->input->post("id", true));
            $img = $this->common->nohtml($this->input->post("img", true));
            ...
            if(empty($_FILES['img']['name'])){
                ...
                unlink(FCPATH.'/uploads/front/about_us/' . $img);
                // name process ------------------------------------------------
                $divide = explode('.', $img);
                $avatar = current($divide);
                // config ------------------------------------------------------
                ...
                if($this->upload->do_upload('img')){
                    ...
                    $data['img'] = $file_name;
                    // save & redirecting --------------------------------------
                    $this->about_us_model->replace_about_us_image($data, $id);
                    $this->session->set_flashdata("globalmsg", lang("sucess_3"));
    ...
    public function service_package() {
        ...
        if($_POST){
            ...
            $this->back_temp->loadCont("back/service_package/service_package.php", array("lists" => $lists));
        }
    }
    ...
    public function replace_image_blog() {
        if ($_POST) {
            // recv post data --------------------------------------------------
            $id = $this->common->nohtml($this->input->post("id", true));
            $title = $this->common->nohtml($this->input->post("title", true));
            // image processor -------------------------------------------------
            if(empty($_FILES['img']['name'])){
                $this->back_temp->error(lang("error_7"));
            }else{
    ...
    public function delete_blog() {
        if ($_POST) {
            $id = $this->common->nohtml($this->input->post("id", true));
            $name = $this->blog_model->get_name($id);
            unlink(FCPATH.'/uploads/front/blog/' . $name->img);
            unlink(FCPATH.'/uploads/front/blog/thumb/' . $name->img);
    ...
    public function delete_news() {
        if ($_POST) {
            $id = $this->common->nohtml($this->input->post("id", true));
            $name = $this->news_model->get_name($id);
            unlink(FCPATH.'/uploads/front/news/' . $name->img);
            unlink(FCPATH.'/uploads/front/news/thumb/' . $name->img);
    ...
    public function delete_events() {
        if ($_POST) {
            $id = $this->common->nohtml($this->input->post("id", true));
            $name = $this->events_model->get_name($id);
            unlink(FCPATH.'/uploads/front/events/' . $name->img);
            unlink(FCPATH.'/uploads/front/events/thumb/' . $name->img);
